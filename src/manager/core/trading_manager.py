...
class TradingManager:
    ...
    def __init__(self, account_configs: List[AccountConfig]):
        ...
        self.account_configs_map: Dict[str, AccountConfig] = {
            acc.account_id: acc for acc in account_configs
        }
    ...
    async def create_trader(self,account_id) ->bool:
        ...
        logger.info(f"Trader Proxy [{account_id}] 初始化完成（enabled: {account_config.enabled}, auto_start: {account_config.auto_start}）")
        return True
    ...
    async def get_strategy(self, account_id: str, strategy_id: str) -> Optional[dict]:
        ...
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request("get_strategy", {"strategy_id": strategy_id})
        return None
    ...
    async def start_strategy(self, account_id: str, strategy_id: str) -> bool:
        ...
        trader = self.traders.get(account_id)
        if trader:
            result = await trader.send_request("start_strategy", {"strategy_id": strategy_id})
    ...
    async def stop_strategy(self, account_id: str, strategy_id: str) -> bool:
        ...
        trader = self.traders.get(account_id)
        if trader:
            result = await trader.send_request("stop_strategy", {"strategy_id": strategy_id})
            return result if result is not None else False
        return False
    ...
    async def start_all_strategies(self, account_id: str) -> bool:
        ...
        trader = self.traders.get(account_id)
        if trader:
            result = await trader.send_request("start_all_strategies", {})
            return result if result is not None else False
        return False
    ...
    async def stop_all_strategies(self, account_id: str) -> bool:
        ...
        trader = self.traders.get(account_id)
        if trader:
            result = await trader.send_request("stop_all_strategies", {})
            return result if result is not None else False
        return False
    ...
    async def get_rotation_instructions(
        self, account_id: str, limit: int = 100, offset: int = 0, status: str = None, enabled: bool = None
    ) -> Optional[dict]:
        """获取换仓指令列表"""
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request(
                "get_rotation_instructions",
                {"limit": limit, "offset": offset, "status": status, "enabled": enabled},
            )
        return None

    async def get_rotation_instruction(self, account_id: str, instruction_id: int) -> Optional[dict]:
        """获取指定换仓指令"""
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request(
    ...
    async def create_rotation_instruction(self, account_id: str, instruction_data: dict) -> Optional[dict]:
        """创建换仓指令"""
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request("create_rotation_instruction", instruction_data)
        return None
    ...
    async def clear_rotation_instructions(self, account_id: str) -> bool:
        ...
        trader = self.traders.get(account_id)
        if trader:
            result = await trader.send_request("clear_rotation_instructions", {})
            return result if result is not None else False
        return False
    ...
    async def import_rotation_instructions(
        ...
        if trader:
            return await trader.send_request(
                "import_rotation_instructions", {"csv_text": csv_text, "filename": filename, "mode": mode}
            )
        return None
    ...
    async def execute_rotation(self, account_id: str) -> bool:
        ...
        trader = self.traders.get(account_id)
        if trader:
            result = await trader.send_request("execute_rotation", {})
            return result if result is not None else False
        return False
    ...
    async def close_all_positions(self, account_id: str) -> bool:
        ...
        trader = self.traders.get(account_id)
        if trader:
            result = await trader.send_request("close_all_positions", {})
            return result if result is not None else False
        return False
    ...
    async def batch_execute_instructions(self, account_id: str, ids: List[int]) -> Optional[dict]:
        ...
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request("batch_execute_instructions", {"ids": ids})
        return None
    ...
    async def batch_delete_instructions(self, account_id: str, ids: List[int]) -> Optional[dict]:
        ...
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request("batch_delete_instructions", {"ids": ids})
        return None
    ...
    async def list_system_params(self, account_id: str, group: Optional[str] = None) -> List:
        ...
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request("list_system_params", {"group": group})
        return []
    ...
    async def get_system_param(self, account_id: str, param_key: str) -> Optional[dict]:
        ...
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request("get_system_param", {"param_key": param_key})
        return None

    async def update_system_param(self, account_id: str, param_key: str, param_value: str) -> Optional[dict]:
        """更新系统参数"""
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request(
    ...
    async def get_system_params_by_group(self, account_id: str, group: str) -> Optional[dict]:
        ...
        trader = self.traders.get(account_id)
        if trader:
            return await trader.send_request("get_system_params_by_group", {"group": group})
        return None
