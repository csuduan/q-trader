...
def init_database(db_path: str = "./storage/trading.db", echo: bool = False) -> Database:
    """
    初始化数据库
...
def get_session() -> Optional[Session]:
    ...
    # 如果数据库未初始化，尝试使用默认路径初始化
    try:
        import os
        default_db_path = os.path.join(os.getcwd(), "storage", "trading.db")
        if os.path.exists(default_db_path):
            db = init_database(default_db_path)
            if db:
                return db.get_session_sync()
    except Exception as e:
        logger.error(f"自动初始化数据库失败: {e}")
